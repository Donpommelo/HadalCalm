...
public class TiledObjectUtil {
    ...
    public static Event parseTiledEvent(PlayState state, MapObject object) {
		...
		switch (object.getName()) {
			...
			case "Alttrigger" -> e = new TriggerAlt(state, object.getProperties().get("message", "", String.class));
			...
			case "Dummy" -> e = new PositionDummy(state, position, size, object.getProperties().get("dummyId", "", String.class));
			...
			case "ScrapSpawn" -> e = new SpawnerScrap(state, position, size, object.getProperties().get("scrap", 0, int.class));
			case "EventClone" -> e = new EventCloner(state, position, size);
			case "EventDelete" -> e = new EventDeleter(state);
			case "EventMove" -> e = new EventMover(state, position, size, object.getProperties().get("gravity", -1.0f, float.class));
    	...
    	for (MovingPoint key : movePointConnections.keySet()) {
    		for (String id : movePointConnections.get(key).split(",")) {
    			if (!id.equals("")) {
        			...
        			//for prefabs, connect to the event parts that are specified to be movable

package com.mygdx.hadal.text;

import com.badlogic.gdx.math.MathUtils;
import com.badlogic.gdx.utils.Array;

/**
 * why did I make this
 * @author Himpanzee Hanicotti
 */
public enum NameGenerator {
	
	//starting consonants. funnier sounding ones have higher frequencies.
	B_START("b", "b_first", 8, "start"),
	C_START("c", "c_first", 4, "start"),
	D_START("d", "d_first", 4, "start"),
	F_START("f", "f_first", 6, "start"),
	G_START("g", "g_first", 6, "start"),
	H_START("h", "h_first", 4, "start"),
	J_START("j", "j_first", 2, "start"),
	K_START("k", "k_first", 2, "start"),
	L_START("l", "l_first", 4, "start"),
	M_START("m", "m_first", 4, "start"),
	N_START("n", "n_first", 4, "start"),
	P_START("p", "p_first", 4, "start"),
	QU_START("qu", "qu_first", "start"),
	R_START("r", "r_first", 4, "start"),
	S_START("s", "s_first", 8, "start"),
	T_START("t", "t_first", 6, "start"),
	V_START("v", "v_first", 2, "start"),
	W_START("w", "w_first", 2, "start"),
	X_START("x", "x_first", "start"),
	Y_START("y", "y_first", 2, "start"),
	Z_START("z", "z_first", "start"),
	
	//second consonant possibilities for things like 'ch', and 'sp'
	C_NEXT("c", "c_next", "s_first"),
	H_NEXT("h", "h_next", "c_first", "g_first", "p_first", "s_first", "t_first", "w_first"),
	K_NEXT("k", "k_next", "s_first"),
	L_NEXT("l", "l_next", "b_first", "c_first", "f_first", "g_first", "k_first", "p_first", "s_first", "v_first"),
	M_NEXT("m", "m_next", "s_first"),
	N_NEXT("n", "n_next", "g_first", "s_first"),
	P_NEXT("p", "p_next", "s_first"),
	Q_NEXT("qu", "qu_next", "s_first"),
	R_NEXT("r", "r_next", "b_first", "c_first", "d_first", "f_first", "g_first", "k_first", "p_first", "t_first", "w_first"),
	T_NEXT("t", "t_next", "s_first"),
	V_NEXT("v", "v_next", "s_first"),
	W_NEXT("w", "w_next", "s_first", "t_first"),
	
	//this is a transition unit that follows the first consonant(s).
	CONSONANT_FIRST("", "consonant_first", "b_first", "c_first", "d_first", "f_first", "g_first", "h_first", "j_first", "k_first", "l_first", "m_first", "n_first", "p_first", "qu_first", "r_first", "s_first", "t_first", "v_first", "w_first", "x_first", "y_first", "z_first"),
	CONSONANT_NEXT("", "consonant_next", "c_next", "h_next", "k_next", "l_next", "m_next", "n_next", "p_next", "qu_next", "r_next", "t_next", "v_next", "w_next"),
	
	//this is a vowel that can follow either starting consonant type
	A_VOWEL("a", "a_first", "consonant_first", "consonant_next"),
	E_VOWEL("e", "e_first", "consonant_first", "consonant_next"),
	I_VOWEL("i", "i_first", "consonant_first", "consonant_next"),
	O_VOWEL("o", "o_first", "consonant_first", "consonant_next"),
	U_VOWEL("u", "u_first", "consonant_first", "consonant_next"),

	//secondary vowels for vowel pairs like "ee", "ea"
	A_SECONDARY("a", "a_secondary", "e_first", "i_first", "o_first", "u_first"),
	E_SECONDARY("e", "e_secondary", "a_first", "e_first", "i_first", "o_first", "u_first"),
	I_SECONDARY("i", "i_secondary", "a_first", "e_first", "o_first", "u_first"),
	O_SECONDARY("o", "o_secondary", "a_first", "e_first", "i_first", "o_first", "u_first"),
	U_SECONDARY("u", "u_secondary", "a_first", "e_first", "i_first", "o_first"),

	VOWEL_FIRST("", "vowel_first", 10, "a_first", "e_first", "i_first", "o_first", "u_first", "a_secondary", "e_secondary", "i_secondary", "o_secondary", "u_secondary"),

	//We can stick another vowel after a universal digram to make things like "clugga" and "farpo"
	A_NEXT("a", "a_next", 60, "universal"),
	E_NEXT("e", "e_next", 40, "universal"),
	I_NEXT("i", "i_next", 40, "universal"),
	O_NEXT("o", "o_next", 60, "universal"),
	U_NEXT("u", "u_next", 50, "universal"),
	VOWEL_NEXT("", "vowel_next", 30, "a_next", "e_next", "i_next", "o_next", "u_next"),
	VOWEL_ANY("", "vowel_any", 20, "vowel_first", "vowel_next"),

	//the weights of these transitions are used to balance probability between names that have long suffixes vs names where each digram is procedurally generated
	A_ANY("", "a", 8, "a_first", "a_next"),
	E_ANY("", "e", 8, "e_first", "e_next"),
	I_ANY("", "i", 8, "i_first", "i_next"),
	O_ANY("", "o", 8, "o_first", "o_next"),
	U_ANY("", "u", 8, "u_first", "u_next"),
	
	//These are digrams that end segments. They can end the name or be followed up with a suffix or another syllable.
	B2("b", "b2", "vowel_first"),
	BB("bb", "bb", "vowel_first"),
	B_("", "b_", "b2", "bb"),

	C2("c", "c2", "vowel_first"),
	CC("cc", "cc", "vowel_first"),
	CH("ch", "ch", "vowel_first"),
	CK("ck", "ck", 3, "vowel_first"),
	CT("ct", "ct", "vowel_first"),
	C_("", "c_", "c2", "cc", "ch", "ck", "ct"),
	
	D2("d", "d2", "vowel_first"),
	DD("dd", "dd", "vowel_first"),
	D_("", "d_", "d2", "dd"),

	F2("f", "f2", "vowel_first"),
	FF("ff", "ff", "vowel_first"),
	FT("ft", "ft", "vowel_first"),
	F_("", "f_", "f2", "ff", "ft"),

	G2("g", "g2", "vowel_first"),
	GG("gg", "gg", 4, "vowel_first"),
	G_("", "g_", "g2", "gg"),

	H2("h", "h_", "vowel_first"),

	J2("j", "j_", "vowel_first"),

	K2("k", "k_", "vowel_first"),

	L2("l", "l2", "vowel_first"),
	LB("lb", "lb", "vowel_first"),
	LC("lc", "lc", "vowel_first"),
	LD("ld", "ld", 3, "vowel_first"),
	LF("lf", "lf", 3, "vowel_first"),
	LG("lg", "lg", 3, "vowel_first"),
	LM("lm", "lm", "vowel_first"),
	LN("ln", "ln", "vowel_first"),
	LP("lp", "lp", 3, "vowel_first"),
	LT("lt", "lt", 2, "vowel_first"),
	LV("lv", "lv", "vowel_first"),
	LZ("lz", "lz", "vowel_first"),
	L_("", "l_", "l2", "lb", "lc", "ld", "lf", "lg", "lm", "ln", "lp", "lt", "lv", "lz"),

	M2("m", "m2", "vowel_first"),
	MB("mb", "mb", "vowel_first"),
	MM("mm", "mm", "vowel_first"),
	MP("mp", "mp", 3, "vowel_first"),
	MS("ms", "ms", "vowel_first"),
	M_("", "m_", "m2", "mb", "mm", "mp", "ms"),

	N2("n", "n2", "vowel_first"),
	NC("nc", "nc", "vowel_first"),
	ND("nd", "nd", 2, "vowel_first"),
	NF("nf", "nf", "vowel_first"),
	NG("ng", "ng", 3, "vowel_first"),
	NJ("nj", "nj", "vowel_first"),
	NK("nk", "nk", 3, "vowel_first"),
	NN("nn", "nn", "vowel_first"),
	NP("np", "np", "vowel_first"),
	NS("ns", "ns", "vowel_first"),
	NT("nt", "nt", "vowel_first"),
	NV("nv", "nv", "vowel_first"),
	NX("nx", "nx", "vowel_first"),
	NZ("nz", "nz", "vowel_first"),
	N_("", "n_", "n2", "nc", "nd", "nf", "ng", "nj", "nk", "nn", "np", "ns", "nt", "nv", "nx", "nz"),

	P2("p", "p2", "vowel_first"),
	PP("pp", "pp", 2, "vowel_first"),
	P_("", "p_", "p2", "pp"),

	R2("r", "r2", "vowel_first"),
	RB("rb", "rb", 2, "vowel_first"),
	RC("rc", "rc", "vowel_first"),
	RD("rd", "rd", 2, "vowel_first"),
	RF("rf", "rf", 2, "vowel_first"),
	RG("rg", "rg", 2, "vowel_first"),
	RJ("rj", "rj", "vowel_first"),
	RK("rk", "rk", 2, "vowel_first"),
	RL("rl", "rl", "vowel_first"),
	RM("rm", "rm", "vowel_first"),
	RN("rn", "rn", "vowel_first"),
	RP("rp", "rp", 2, "vowel_first"),
	RR("rr", "rr", 2, "vowel_first"),
	RS("rs", "rs", "vowel_first"),
	RT("rt", "rt", 2, "vowel_first"),
	RV("rv", "rv", "a", "vowel_first"),
	RX("rx", "rx", "vowel_first"),
	RZ("rz", "rz", "vowel_first"),
	R_("", "r_", "r2", "rb", "rc", "rd", "rf", "rg", "rj", "rk", "rl", "rm", "rn", "rp", "rr", "rs", "rt", "rv", "rx", "rz"),

	S2("s", "s2", "vowel_first"),
	SC("sc", "sc", "vowel_first"),
	SH("sh", "sh", 2, "vowel_first"),
	SK("sk", "sk", "vowel_first"),
	SP("sp", "sp", "vowel_first"),
	SS("ss", "ss", "vowel_first"),
	ST("st", "st", "vowel_first"),
	S_("", "s_", "s2", "sc", "sh", "sk", "sp", "ss", "st"),

	T2("t", "t2", "vowel_first"),
	TH("th", "th", "vowel_first"),
	TS("ts", "ts", "vowel_first"),
	TT("tt", "tt", 2, "vowel_first"),
	TZ("tz", "tz", "vowel_first"),
	T_("", "t_", "t2", "th", "ts", "tt", "tz"),

	V2("v", "v_", "vowel_first"),

	W2("w", "w2", "vowel_first"),
	WB("wb", "wb", "vowel_first"),
	WD("wd", "wd", "vowel_first"),
	WK("wk", "wk", "vowel_first"),
	WN("wn", "wn", "vowel_first"),
	WP("wp", "wp", "vowel_first"),
	W_("", "w_", "w2", "wb", "wc", "wd", "wk", "wn", "wp"),

	X2("x", "x2", "vowel_first"),
	XT("xt", "xt", "vowel_first"),
	X_("", "x_", "x2", "xt"),

	Y2("y", "y_", "vowel_first"),

	Z2("z", "z2", "vowel_first"),
	ZZ("zz", "zz", "vowel_first"),
	Z_("", "z_", "z2", "zz"),

	//a "universal first syllable" is something like "gronk" or "snark"
	UNIVERSAL_FIRST_SYLLABLE("", "universal", 50, "b_", "c_", "d_", "f_", "g_", "h_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),

	//these things follow a universal first syllable + a vowel (makes something like "blarter" or "plunkle") and can lead to another suffix
	DGE("dge", "dge", "vowel_next"),
	_L("l", "_l", "vowel_next"),
	_N("n", "_n", "vowel_next"),
	_NG("ng", "_ng", "vowel_next"),
	_R("r", "_r", "vowel_next", "le"),
	LE("le", "le", "b_", "c_", "d_", "f_", "g_", "h_", "j_", "k_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),

	MISC_END("", "misc", 10, "dge", "le", "_l", "_n", "_ng", "_r"),

	//specific suffixes that can follow a digram ending in certain consonants
	ACH("ach", "end", "misc", "universal"),
	ALOV("alov", "end", "universal"),
	ANNE("anne", "end", "misc", "universal"),
	ANOV("anov", "end", "universal"),
	APPA("appa", "end", "universal"),
	ART("art", "end", "misc", "universal"),
	AWAY("away", "end", "misc", "universal"),

	UNIVERSAL_MINUS_B("", "universal_no_b", 25, "c_", "d_", "f_", "g_", "h_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	BACH("bach", "end", "vowel_any", "misc", "universal_no_b"),
	BAT("bat", "end", "vowel_any", "misc", "universal_no_b"),
	BAUER("bauer", "end", "vowel_any", "misc", "universal_no_b"),
	BAUM("baum", "end", "vowel_any", "misc", "universal_no_b"),
	BEAN("bean", "end", "vowel_any", "misc", "universal_no_b"),
	BECK("beck", "end", "vowel_any", "misc", "universal_no_b"),
	BENDER("bender", "end", "vowel_any", "misc", "universal_no_b"),
	BERG("berg", "end", "vowel_any", "misc", "universal_no_b"),
	BERRY("berry", "end", "vowel_any", "misc", "universal_no_b"),
	BERT("bert", "end", "vowel_any", "misc", "universal_no_b"),
	BIRD("bird", "end", "vowel_any", "misc", "universal_no_b"),
	BLATT("blatt", "end", "vowel_any", "misc", "universal_no_b"),
	BLOOM("bloom", "end", "vowel_any", "misc", "universal_no_b"),
	BLORT("blort", "end", "vowel_any", "misc", "universal_no_b"),
	BLUD("blud", "end", "vowel_any", "misc", "universal_no_b"),
	BONNIER("bonnier", "end", "vowel_any", "misc", "universal_no_b"),
	BOO("boo", "end", "vowel_any", "misc", "universal_no_b"),
	BOROUGH("borough", "end", "vowel_any", "misc", "universal_no_b"),
	BORG("borg", "end", "vowel_any", "misc", "universal_no_b"),
	BORT("bort", "end", "vowel_any", "misc", "universal_no_b"),
	BOTTOMS("bottoms", "end", "vowel_any", "misc", "universal_no_b"),
	BOURNE("bourne", "end", "vowel_any", "misc", "universal_no_b"),
	BRINGER("bringer", "end", "vowel_any", "misc", "universal_no_b"),
	BROOKE("brooke", "end", "vowel_any", "misc", "universal_no_b"),
	BUCKET("bucket", "end", "vowel_any", "misc", "universal_no_b"),
	BUD("bud", "end", "vowel_any", "misc", "universal_no_b"),
	BUDDY("buddy", "end", "vowel_any", "misc", "universal_no_b"),
	BURY("bury", "end", "vowel_any", "misc", "universal_no_b"),
	BURG("burg", "end", "vowel_any", "misc", "universal_no_b"),
	BURGER("burger", "end", "vowel_any", "misc", "universal_no_b"),
	BY("by", "end", "vowel_any", "misc", "universal_no_b"),

	UNIVERSAL_MINUS_C("", "universal_no_c", 10, "b_", "d_", "f_", "g_", "h_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	CAKE("cake", "end", "vowel_any", "misc", "universal_no_c"),
	CASTER("caster", "end", "vowel_any", "misc", "universal_no_c"),
	CHAMP("champ", "end", "vowel_any", "misc", "universal_no_c"),
	CHESTER("chester", "end", "vowel_any", "misc", "universal_no_c"),
	CLIFF("cliff", "end", "vowel_any", "misc", "universal_no_c"),
	COCK("cock", "end", "vowel_any", "misc", "universal_no_c"),
	COIN("coin", "end", "vowel_any", "misc", "universal_no_c"),
	COFF("coff", "end", "vowel_any", "misc", "universal_no_c"),
	CORK("cork", "end", "vowel_any", "misc", "universal_no_c"),
	CORN("corn", "end", "vowel_any", "misc", "universal_no_c"),
	COTT("cott", "end", "vowel_any", "misc", "universal_no_c"),
	COURT("court", "end", "vowel_any", "misc", "universal_no_c"),
	CRAFT("craft", "end", "vowel_any", "misc", "universal_no_c"),
	CROIX("croix", "end", "vowel_any", "misc", "universal_no_c"),

	UNIVERSAL_MINUS_D("", "universal_no_d", 15, "b_", "c_", "f_", "g_", "h_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	DALE("dale", "end", "vowel_any", "misc", "universal_no_d"),
	DATTER("datter", "end", "vowel_any", "misc", "universal_no_d"),
	DELL("dell", "end", "vowel_any", "misc", "universal_no_d"),
	DEN("den", "end", "vowel_any", "misc", "universal_no_d"),
	DERF("derf", "end", "vowel_any", "misc", "universal_no_d"),
	DEW("dew", "end", "vowel_any", "misc", "universal_no_d"),
	DIN("din", "end", "vowel_any", "misc", "universal_no_d"),
	DON("don", "end", "vowel_any", "misc", "universal_no_d"),
	DONALD("donald", "end", "vowel_any", "misc", "universal_no_d"),
	DOOF("doof", "end", "vowel_any", "misc", "universal_no_d"),
	DORF("dorf", "end", "vowel_any", "misc", "universal_no_d"),
	DORP("dorp", "end", "vowel_any", "misc", "universal_no_d"),
	DRAGON("dragon", "end", "vowel_any", "misc", "universal_no_d"),
	DUL("dul", "end", "vowel_any", "misc", "universal_no_d"),
	DUNG("dung", "end", "vowel_any", "misc", "universal_no_d"),
	DURF("durf", "end", "vowel_any", "misc", "universal_no_d"),

	EATER("eater", "end", "misc", "universal"),
	ETTE("ette", "end", "misc", "universal"),

	UNIVERSAL_MINUS_F("", "universal_no_f", 10, "b_", "c_", "d_", "g_", "h_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	FACE("face", "end", "vowel_any", "misc", "universal_no_f"),
	FAM("fam", "end", "vowel_any", "misc", "universal_no_f"),
	FAX("fax", "end", "vowel_any", "misc", "universal_no_f"),
	FELL("fell", "end", "vowel_any", "misc", "universal_no_f"),
	FIELD("field", "end", "vowel_any", "misc", "universal_no_f"),
	FINDER("finder", "end", "vowel_any", "misc", "universal_no_f"),
	FISH("fish", "end", "vowel_any", "misc", "universal_no_f"),
	FISHER("fisher", "end", "vowel_any", "misc", "universal_no_f"),
	FIRE("fire", "end", "vowel_any", "misc", "universal_no_f"),
	FOLD("fold", "end", "vowel_any", "misc", "universal_no_f"),
	FOLK("folk", "end", "vowel_any", "misc", "universal_no_f"),
	FOO("foo", "end", "vowel_any", "misc", "universal_no_f"),
	FOOT("foot", "end", "vowel_any", "misc", "universal_no_f"),
	FORD("ford", "end", "vowel_any", "misc", "universal_no_f"),
	FORK("fork", "end", "vowel_any", "misc", "universal_no_f"),
	FREE("free", "end", "vowel_any", "misc", "universal_no_f"),
	FRIEND("friend", "end", "vowel_any", "misc", "universal_no_f"),
	FRUIT("fruit", "end", "vowel_any", "misc", "universal_no_f"),
	FRY("fry", "end", "vowel_any", "misc", "universal_no_f"),
	FUL("ful", "end", "vowel_any", "misc", "universal_no_f"),

	UNIVERSAL_MINUS_G("", "universal_no_g", 5, "b_", "c_", "d_", "f_", "h_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	GETTE("gette", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GILL("gill", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GINS("gins", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GOLD("gold", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GRAIN("grain", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GRASS("grass", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GRAVE("grave", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),
	GRAVY("gravy", "end", "vowel_any", "dge", "le", "_r","universal_no_g"),

	UNIVERSAL_MINUS_H("", "universal_no_h", 5, "b_", "c_", "d_", "f_", "g_", "j_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	HAM("ham", "end", "vowel_any", "misc", "universal_no_h"),
	HAUTZ("ham", "end", "vowel_any", "misc", "universal_no_h"),
	HAZ("haz", "end", "vowel_any", "misc", "universal_no_h"),
	HAMMER("hammer", "end", "vowel_any", "misc", "universal_no_h"),
	HAND("hand", "end", "vowel_any", "misc", "universal_no_h"),
	HEAD("head", "end", "vowel_any", "misc", "universal_no_h"),
	HEART("heart", "end", "vowel_any", "misc", "universal_no_h"),
	HEIMER("heimer", "end", "vowel_any", "misc", "universal_no_h"),
	HOFF("hoff", "end", "vowel_any", "misc", "universal_no_h"),
	HOG("hog", "end", "vowel_any", "misc", "universal_no_h"),
	HOLE("hole", "end", "vowel_any", "misc", "universal_no_h"),
	HORN("horn", "end", "vowel_any", "misc", "universal_no_h"),
	HORT("hort", "end", "vowel_any", "misc", "universal_no_h"),

	IE("ie", "end", "universal"),
	INOV("inov", "end", "universal"),
	ISH("ish", "end", "universal"),

	UNIVERSAL_MINUS_K("", "universal_no_k", 5, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	KAMP("kins", "end", "vowel_any", "misc", "universal_no_k"),
	KINS("kins", "end", "vowel_any", "misc", "universal_no_k"),
	KIRK("kirk", "end", "vowel_any", "misc", "universal_no_k"),
	KOFF("koff", "end", "vowel_any", "misc", "universal_no_k"),
	KRANTZ("krantz", "end", "vowel_any", "misc", "universal_no_k"),
	KURTZ("kurtz", "end", "vowel_any", "misc", "universal_no_k"),

	UNIVERSAL_MINUS_L("", "universal_no_l", 10, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	LACE("lace", "end", "vowel_any", "misc", "universal_no_l"),
	LAMB("lamb", "end", "vowel_any", "misc", "universal_no_l"),
	LAND("land", "end", "vowel_any", "misc", "universal_no_l"),
	LARD("lard", "end", "vowel_any", "misc", "universal_no_l"),
	LEY("ley", "end", "vowel_any", "misc", "universal_no_l"),
	LIP("lip", "end", "vowel_any", "misc", "universal_no_l"),
	LORD("lord", "end", "vowel_any", "misc", "universal_no_l"),
	LOVE("love", "end", "vowel_any", "misc", "universal_no_l"),
	LUMP("lump", "end", "vowel_any", "misc", "universal_no_l"),

	UNIVERSAL_MINUS_M("", "universal_no_m", 10, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	MAN("man", "end", "vowel_any", "universal_no_m"),
	MAKER("maker", "end", "vowel_any", "misc", "universal_no_m"),
	MARK("mark", "end", "vowel_any", "misc", "universal_no_m"),
	MEISTER("meister", "end", "vowel_any", "misc", "universal_no_m"),
	MEYER("meyer", "end", "vowel_any", "misc", "u", "universal_no_m"),
	MILL("mill", "end", "vowel_any", "misc", "u", "universal_no_m"),
	MINER("miner", "end", "vowel_any", "misc", "u", "universal_no_m"),
	MINT("mint", "end", "vowel_any", "misc", "u", "universal_no_m"),
	MONGER("monger", "end", "vowel_any", "misc", "u", "universal_no_m"),
	MOSS("moss", "end", "vowel_any", "misc", "u", "universal_no_m"),
	MUFFIN("muffin", "end", "vowel_any", "misc", "universal_no_m"),

	UNIVERSAL_MINUS_N("", "universal_no_n", 10, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	NADO("nado", "end", "vowel_any", "misc", "universal_no_n"),
	NARD("nard", "end", "vowel_any", "misc", "universal_no_n"),
	NEAUX("neaux", "end", "vowel_any", "misc", "universal_no_n"),
	NER("ner", "end", "vowel_any", "misc", "universal_no_n"),
	NERD("nerd", "end", "vowel_any", "misc", "universal_no_n"),
	NORD("nord", "end", "vowel_any", "misc", "universal_no_n"),
	NOSE("nose", "end", "vowel_any", "misc", "universal_no_n"),
	NUT("nut", "end", "vowel_any", "misc", "universal_no_n"),
	
	OE("oe", "end", "universal"),
	OO("oo", "end", "universal"),
	OOEY("ooey", "end", "universal"),

	UNIVERSAL_MINUS_P("", "universal_no_p", 10, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "r_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	PANTS("pants", "end", "vowel_any", "misc", "universal_no_p"),
	PEPPER("pepper", "end", "vowel_any", "misc", "universal_no_p"),
	PIG("pig", "end", "vowel_any", "misc", "universal_no_p"),
	PON("pon", "end", "vowel_any", "misc", "universal_no_p"),
	POOF("poof", "end", "vowel_any", "misc", "universal_no_p"),
	POSTER("poster", "end", "vowel_any", "misc", "universal_no_p"),
	POULOS("poulos", "end", "vowel_any", "misc", "universal_no_p"),
	PROOF("proof", "end", "vowel_any", "misc", "universal_no_p"),
	PUFF("puff", "end", "vowel_any", "misc", "universal_no_p"),

	UNIVERSAL_MINUS_R("", "universal_no_r", 5, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "p_", "s_", "t_", "v_", "w_", "x_", "y_", "z_"),
	RAIN("rain", "end", "vowel_any", "universal_no_r"),
	REICH("reich", "end", "vowel_any", "universal_no_r"),
	RICH("rich", "end", "vowel_any", "misc", "universal_no_r"),
	ROOT("root", "end", "vowel_any", "misc", "universal_no_r"),
	ROTH("roth", "end", "vowel_any", "misc", "universal_no_r"),

	UNIVERSAL_MINUS_S("", "universal_no_s", 20, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "p_", "r_", "t_", "v_", "w_", "x_", "y_", "z_"),
	SALT("salt", "end", "vowel_any", "misc", "universal_no_s"),
	SAND("sand", "end", "vowel_any", "misc", "universal_no_s"),
	SAP("sap", "end", "vowel_any", "misc", "universal_no_s"),
	SHIRE("shire", "end", "vowel_any", "misc", "universal_no_s"),
	SHOES("shoes", "end", "vowel_any", "misc", "universal_no_s"),
	SINGER("singer", "end", "vowel_any", "misc", "universal_no_s"),
	SKULL("skull", "end", "vowel_any", "misc", "universal_no_s"),
	SKY("sky", "end", "vowel_any", "misc", "universal_no_s"),
	SLINGER("slinger", "end", "vowel_any", "misc", "universal_no_s"),
	SMITH("smith", "end", "vowel_any", "misc", "universal_no_s"),
	SOAP("soap", "end", "vowel_any", "misc", "universal_no_s"),
	SOCKS("socks", "end", "vowel_any", "misc", "universal_no_s"),
	SON("son", "end", "vowel_any", "misc", "universal_no_s"),
	SPICE("spice", "end", "vowel_any", "misc", "universal_no_s"),
	SPOON("spice", "end", "vowel_any", "misc", "universal_no_s"),
	STEIN("stein", "end", "vowel_any", "misc", "universal_no_s"),
	STON("ston", "end", "vowel_any", "misc", "universal_no_s"),
	SUCK("suck", "end", "vowel_any", "misc", "universal_no_s"),
	SY("sy", "end", "vowel_any", "misc", "universal_no_s"),

	UNIVERSAL_MINUS_T("", "universal_no_t", 10, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "v_", "w_", "x_", "y_", "z_"),
	TON("ton", "end", "vowel_any", "misc", "universal_no_t"),
	TORP("torp", "end", "vowel_any", "misc", "universal_no_t"),
	TOUNGE("tounge", "end", "vowel_any", "misc", "universal_no_t"),
	THROP("throp", "end", "vowel_any", "misc", "universal_no_t"),
	THWAITE("thwaite", "end", "vowel_any", "misc", "universal_no_t"),

	UNIVERSAL_MINUS_V("", "universal_no_v", "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "w_", "x_", "y_", "z_"),
	VICH("vich", "end", "vowel_any", "misc", "universal_no_v"),
	VILLE("ville", "end", "vowel_any", "misc", "universal_no_v"),
	VON("von", "end", "vowel_any", "misc", "universal_no_v"),

	UNIVERSAL_MINUS_W("", "universal_no_w", 20, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "x_", "y_", "z_"),
	WAD("wad", "end", "vowel_any", "misc", "universal_no_w"),
	WALD("wald", "end", "vowel_any", "misc", "universal_no_w"),
	WALL("wall", "end", "vowel_any", "misc", "universal_no_w"),
	WATER("water", "end", "vowel_any", "misc", "universal_no_w"),
	WERTZ("wertz", "end", "vowel_any", "misc", "universal_no_w"),
	WICH("wich", "end", "vowel_any", "misc", "universal_no_w"),
	WICK("wick", "end", "vowel_any", "misc", "universal_no_w"),
	WIG("wig", "end", "vowel_any", "misc", "universal_no_w"),
	WIN("win", "end", "vowel_any", "misc", "universal_no_w"),
	WITZ("witz", "end", "vowel_any", "misc", "universal_no_w"),
	WOCK("wock", "end", "vowel_any", "misc", "universal_no_w"),
	WOOD("wood", "end", "vowel_any", "misc", "universal_no_w"),
	WORF("worf", "end", "vowel_any", "misc", "universal_no_w"),
	WORT("wort", "end", "vowel_any", "misc", "universal_no_w"),
	WORTH("worth", "end", "vowel_any", "misc", "universal_no_w"),
	WRANGLER("wrangler", "end", "vowel_any", "misc","universal_no_w"),
	WRASSLER("wrassler", "end", "vowel_any", "misc", "universal_no_w"),
	WUMP("wump", "end", "vowel_any", "misc", "universal_no_w"),

	Y_END("y", "end", "a", "e", "o", "misc", "universal"),
	YANOV("yanov", "end", "misc", "universal"),

	UNIVERSAL_MINUS_Z("", "universal_no_z", 5, "b_", "c_", "d_", "f_", "g_", "j_", "h_", "k_", "l_", "m_", "n_", "p_", "r_", "s_", "t_", "v_", "w_", "x_", "y_"),
	ZAP("zap", "end", "vowel_any", "misc", "universal_no_z"),
	ZI("zi", "end", "vowel_any", "misc", "universal_no_z"),
	ZOG("zog", "end", "vowel_any", "misc", "universal_no_z"),
	ZOOKS("zooks", "end", "vowel_any", "misc", "universal_no_z"),

	_NOTHING_("", "end", 25, "a_next", "o_next", "misc", "universal"),

	//misc end suffixes
	ACAMOLE("acamole", "end", "u"),
	ADRUNNER("adrunner", "end", "o"),
	ADULUPE("adulupe", "end", "u"),
	AMOFMUSHROOM("amofmushroom", "end", "e"),
	ANDIP("andip", "end", "e"),
	ANUTBUTTER("anutbutter", "end", "e"),
	ARDVARK("ardvark", "end", "a"),
	ARL("arl", "end", "e"),
	ASTBEEF("astbeef", "end", "o"),
	ATBALL("atball", "end", "e"),
	ATMEAL("atmeal", "end", "o"),
	ATRICE("atrice", "end", "e"),
	AVA("ava", "end", "i", "o", "u"),
	AZEBUCKET("azebucket", "end", "e"),
	
	BAGAIL("bagail", "end", "a"),
	BAGANOUSH("baganoush", "end", "a", "o", "u"),
	BALANTE("balante", "end", "a", "e", "i", "o", "u"),
	BAMA("bama", "end", "o"),
	BANERO("banero", "end", "a"),
	BANFOO("banfoo", "end", "a", "e", "i", "o", "u"),
	BASCO("basco", "end", "a"),
	BASTIAN("bastian", "end", "a", "e", "i", "o", "u"),
	BBADON("bbadon", "end", "a", "e", "i", "o", "u"),
	BBERER("bberer", "end", "a", "e", "i", "o", "u"),
	BBERISH("bberish", "end", "a", "e", "i", "o", "u"),
	BBERY("bbery", "end", "a", "e", "i", "o", "u"),
	BBIB("bbib", "end", "a"),
	BBITY("bbity", "end", "a", "e", "i", "o", "u"),
	BBONDONATO("bbondonato", "end", "a"),
	BBOTT("bbott", "end", "a", "e", "i", "o", "u"),
	BDUL("bdul", "end", "a", "e", "i", "o", "u"),
	BEDIAH("bediah", "end", "a", "e", "i", "o", "u"),
	BEDNEGO("bednego", "end", "a", "e", "i", "o", "u"),
	BENEZER("benezer", "end", "e"),
	BERDASH("berdash", "end", "a", "e", "i", "o", "u"),
	BEYE("beye", "end", "i"),
	BIDEEN("bideen", "end", "a", "e", "i", "o", "u"),
	BIO("bio", "end", "a"),
	BISCUS("biscus", "end", "a", "e", "i", "o", "u"),
	BISH("bish", "end", "a", "e", "i", "o", "u"),
	BITHA("bitha", "end", "a", "e", "i", "o", "u"),
	BOLA("bola", "end", "e"),
	BOON("boon", "end", "a"),
	BRAMANIAM("bramaniam", "end", "u"),
	BRAXAS("braxas", "end", "a"),
	BRINA("brina", "end", "a", "e", "i", "o", "u"),
	BRIANA("briana", "end", "a", "e", "i", "o", "u"),
	BRUARY("bruary", "end", "a", "e", "i", "o", "u"),
	BUTICABA("buticaba", "end", "a"),
	BYFACE("byface", "end", "a"),

	CANDCHEESE("candcheese", "end", "a"),
	CACIA("cacia", "end", "a", "e", "i", "o", "u"),
	CAINE("caine", "end", "o"),
	CAMA("cama", "end", "i"),
	CARDO("cardo", "end", "a", "e", "i", "o", "u"),
	CAQUE("caque", "end", "a"),
	CASTE("caste", "end", "a", "o"),
	CATINI("catini", "end", "a", "i", "o", "u"),
	CCACIO("ccacio", "end", "a", "e", "i", "o", "u"),
	CCIATELLO("cciatello", "end", "a", "e", "i", "o", "u"),
	CCINI("ccini", "end", "a", "e", "i", "o", "u"),
	CEROLA("cerola", "end", "a"),
	CHABOD("chabod", "end", "i"),
	CHARD("chard", "end", "a", "e", "i", "o", "u"),
	CHARY("chary", "end", "a", "e", "i", "o", "u"),
	CHELLE("chelle", "end", "a", "e", "i", "o", "u"),
	CHERTORTE("chertorte", "end", "a"),
	CHNOLD("chnold", "end", "a", "e", "i", "o", "u"),
	CHYSOISSE("chysoisse", "end", "i"),
	CIEWEITZ("cieweitz", "end", "a", "e", "i", "o", "u"),
	CKABYE("ckabye", "end", "o", "u"),
	CKADEE("ckadee", "end", "a", "e", "i", "o", "u"),
	CKENSPIEL("ckenspiel", "end", "o"),
	CKER("cker", "end", "a", "e", "i", "o", "u"),
	CKERAL("ckeral", "end", "a"),
	CKETT("ckett", "end", "a", "e", "i", "o", "u"),
	CKHEART("ckheart", "end", "a", "e", "i", "o", "u"),
	CKINGBIRD("ckingbird", "end", "a", "e", "i", "o", "u"),
	CKLEY("ckley", "end", "o"),
	CKWHEAT("ckwheat", "end", "u"),
	CNICBASKET("cnicbasket", "end", "a", "e", "i", "o", "u"),
	CODILE("codile", "end", "o"),
	CONUT("conut", "end", "o"),
	CORY("cory", "end", "i"),
	CORICE("corice", "end", "i"),
	CRETIUS("cretius", "end", "a", "e", "i", "o", "u"),
	CTAVIO("ctavio", "end", "a", "e", "i", "o", "u"),
	CUMBER("cumber", "end", "u"),
	CUSPOCUS("cuspocus", "end", "o"),
	CYCLE("cycle", "end", "i"),
	
	DADDLE("daddle", "end", "e", "i"),
	DAH("dah", "end", "a", "e", "i", "o", "u"),
	DALUPE("dalupe", "end", "a"),
	DARDAR("dardar", "end", "a"),
	DARDUS("dardus", "end", "a", "e", "i", "o", "u"),
	DAYAT("dayat", "end", "e"),
	DETTE("dette", "end", "o"),
	DDAD("ddad", "end", "a", "e", "i", "o", "u"),
	DDAM("ddam", "end", "a", "e", "i", "o", "u"),
	DDARTH("ddarth", "end", "i"),
	DDEUS("ddeus", "end", "a", "e", "i", "o", "u"),
	DDING("dding", "end", "a", "e", "i", "o", "u"),
	DDJOB("ddjob", "end", "o"),
	DELAIDA("delaida", "end", "a"),
	DELAIRE("delaire", "end", "a", "e", "i", "o", "u"),
	DEON("deon", "end", "a", "e", "i", "o", "u"),
	DERICK("derick", "end", "a", "e", "i", "o", "u"),
	DGER("dger", "end", "a", "o"),
	DGERIDOO("dgeridoo", "end", "e", "i"),
	DIBRANCH("dibranch", "end", "u"),
	DICCHIO("diccio", "end", "a"),
	DICULOUS("diculous", "end", "a", "e", "i", "o", "u"),
	DIMIR("dimir", "end", "a", "e", "i", "o", "u"),
	DISON("dison", "end", "a"),
	DITYA("ditya", "end", "a"),
	DJA("dja", "end", "a"),
	DLOW("dlow", "end", "a", "e", "i", "o", "u"),
	DOLPH("dolph", "end", "a", "o", "u"),
	DOVICO("dovico", "end", "u"),
	DRACH("drach", "end", "a", "o", "u"),
	DRAMALECH("dramalech", "end", "a"),
	DSWALLOP("dswallop", "end", "o"),
	DUARDO("duardo", "end", "a", "o", "u"),
	DUH("duh", "end", "a", "e", "i", "o", "u"),
	DYPOO("dypoo", "end", "o", "u"),
	DZILLA("dzilla", "end", "o"),
	
	EBIRD("ubird", "end", "u"),
	EBONE("ebone", "end", "e"),
	EFEATER("efeater", "end", "e"),
	EFUS("efus", "end", "e"),
	EP("ep", "end", "e"),
	EPAK("epak", "end", "e"),
	EPLE("eple", "end", "e"),
	ERIO("erio", "end", "e"),
	ESESTEAK("esesteak", "end", "e"),
	ESHMA("eshma", "end", "a"),
	ETCORN("etcorn", "end", "e"),
	EZLEBUB("ezlebub", "end", "e"),
	EZY("ezy", "end", "e"),

	FAEL("fael", "end", "a", "e", "i", "o", "u"),
	FKA("fka", "end", "a", "e", "i", "o", "u"),
	FFALO("ffalo", "end", "a", "e", "i", "o", "u"),
	FFERVESCENCE("ffervescence", "end", "e"),
	FFERSON("fferson", "end", "a", "e", "i", "o", "u"),
	FFERTY("fferty", "end", "a", "e", "i", "o", "u"),
	FFINITY("ffinity", "end", "a", "o", "u"),
	FFONSO("ffonso", "end", "a", "e", "i", "o", "u"),
	FRODITE("frodite", "end", "a"),

	GAMEET("gameet", "end", "a", "e", "i", "o", "u"),
	GARFREE("garfree", "end", "u"),
	GARTH("garth", "end", "a", "e", "i", "o", "u"),
	GATHA("gatha", "end", "a"),
	GATRON("gatron", "end", "a", "e", "i", "o", "u"),
	GAWA("gawa", "end", "a", "e", "i", "o", "u"),
	GBIRD("gbird", "end", "i"),
	GDALENA("gdalena", "end", "a", "e", "i", "o", "u"),
	GDANOFF("gdanoff", "end", "a", "e", "i", "o", "u"),
	GEY("gey", "end", "o"),
	GGDROP("ggdrop", "end", "e"),
	GGER("gger", "end", "a", "e", "i", "o", "u"),
	GGINBOTHAM("gginbotham", "end", "a", "e", "i", "o", "u"),
	GGPLANT("ggplant", "end", "e"),
	GHAV("ghav", "end", "a"),
	GHERTY("gherty", "end", "a", "e", "i", "o", "u"),
	GHETTI("ghetti", "end", "a"),
	GHTJAR("ghtjar", "end", "i"),
	GINALD("ginald", "end", "a", "e", "i", "o", "u"),
	GLIADO("gliado", "end", "a"),
	GLODYTE("glodyte", "end", "a","o", "u"),
	GMA("gma", "end", "a", "e", "i", "o", "u"),
	GMAC("gmac", "end", "e", "i"),
	GMAROLE("gmarole", "end", "i"),
	GNATIO("gnatio", "end", "a", "e", "i", "o", "u"),
	GNES("gnes", "end", "a"),
	GNIFICANT("gnificant", "end", "i"),
	GOR("gor", "end", "a", "e", "i", "o", "u"),
	GORY("gory", "end", "a", "e", "i", "o", "u"),
	GPIE("gpie", "end", "a"),
	GRIPPA("grippa", "end", "a"),
	GUANA("guana", "end", "a", "e", "i", "o", "u"),
	GUEL("guel", "end", "a", "e", "i", "o", "u"),
	GWUMP("gwump", "end", "a", "e", "i", "o", "u"),

	HAMMAD("hammad", "end", "a", "o", "u"),
	HANNON("hannon", "end", "o"),
	HEMOTH("hemoth", "end", "e"),
	HIO("hio", "end", "o"),
	HJIT("hjit", "end", "a"),
	HLRABI("hlrabi", "end", "o"),
	HRIMAN("hriman", "end", "a"),

	ID("id", "end", "o"),
	IGLEBOND("iglebond", "end", "e"),
	ILEEN("ileen", "end", "e"),
	INSLEY("insley", "end", "a"),
	IQUIRI("iquiri", "end", "a"),
	ISEDPORK("isedpork", "end", "a"),
	ITBASKET("itbasket", "end", "u"),
	ITCAKE("itcake", "end", "u"),

	JANDRO("jandro", "end", "a", "e", "i", "o", "u"),
	JEED("jandro", "end", "a", "u"),
	JESH("jandro", "end", "a"),
	JITO("jito", "end", "o"),
	JUBE("jube", "end", "u"),

	KA("ka", "end", "a", "e", "i", "o", "u"),
	KANORI("kanori", "end", "a"),
	KASHA("kasha", "end", "a"),
	KACHU("kachu", "end", "i"),
	KCHOY("kchoy", "end", "o"),
	KEDPOTATO("kedpotato", "end", "a"),
	KEEM("keem", "end", "a"),
	KKAIL("kkail", "end", "i"),
	KLAVA("klava", "end", "a"),
	KOLA("kola", "end", "a", "e", "i", "o", "u"),
	KORA("kora", "end", "a", "e", "i", "o", "u"),
	KSHASA("kshasa", "end", "a"),
	KSHUKA("kshuka", "end", "a"),
	KRA("kra", "end", "a", "o"),
	KULELE("kulele", "end", "u"),
	KURAI("kurai", "end", "a"),

	LABASH("labash", "end", "a"),
	LAF("laf", "end", "o"),
	LAFEL("lafel", "end", "a"),
	LAMARI("lamari", "end", "a"),
	LAMMBO("lammbo", "end", "a"),
	LAMI("lami", "end", "a"),
	LAMONTE("lamonte", "end", "a", "e", "o", "u"),
	LANDA("landa", "end", "a", "e", "i", "o", "u"),
	LANTRO("lantro", "end", "i"),
	LAPENO("lapeno", "end", "a"),
	LARBUS("larbus", "end", "a", "e", "i", "o", "u"),
	LARIO("lario", "end", "a", "e", "i", "o", "u"),
	LARKEY("larkey", "end", "a"),
	LATINOUS("latinous", "end", "e"),
	LAZAR("lazar", "end", "a", "e"),
	LBACORE("lbacore", "end", "a"),
	LBATROSS("lbatross", "end", "a", "e", "i", "o", "u"),
	LBERO("lboro", "end", "a", "e", "i", "o", "u"),
	LBERRY("lberry", "end", "u"),
	LCHER("lcher", "end", "a", "e", "i", "o", "u"),
	LCIMER("lcimer", "end", "o", "u"),
	LDEBEAST("ldebeast", "end", "e", "i"),
	LDEMAR("ldemar", "end", "a", "e", "i", "o", "u"),
	LDERDASH("lderdash", "end", "a", "e", "i", "o", "u"),
	LDOROR("ldoror", "end", "a"),
	LDOUS("ldous", "end", "a", "e", "i", "o", "u"),
	LDRICH("ldrich", "end", "a", "e", "i", "o", "u"),
	LDRIN("ldrin", "end", "a"),
	LDUGGERY("lduggery", "end", "u"),
	LDWIN("ldwin", "end", "a", "e", "i", "o", "u"),
	LEANOR("leanor", "end", "e"),
	LECTRA("lectra", "end", "e"),
	LEMACHUS("lemachus", "end", "a", "e", "i", "o", "u"),
	LENDULA("lendula", "end", "a"),
	LEPHANT("lephant", "end", "e"),
	LESLAW("leslaw", "end", "o"),
	LESTE("leste", "end", "e"),
	LEXANDER("lexander", "end", "a", "e", "i", "o", "u"),
	LFANHUI("lfanhui", "end", "a"),
	LGA("lga", "end", "a", "e", "i", "o", "u"),
	LGAKOV("lgakov", "end", "a", "o", "u"),
	LHOUETTE("lhouette", "i"),
	LIEZER("liezer", "e"),
	LIMANJARO("limanjaro", "end", "i"),
	LFREDO("lfredo", "end", "a", "e", "i", "o", "u"),
	LFRAM("lfram", "end", "a", "e", "i", "o", "u"),
	LGERNON("lgernon", "end", "a", "o", "u"),
	LICAN("lican", "end", "a", "e", "i", "o", "u"),
	LICIOUS("licious", "end", "a", "e", "i", "o", "u"),
	LIA("lia", "end", "a", "e", "i", "o", "u"),
	LIAS("lias", "end", "a", "e", "i", "o", "u"),
	LINDA("linda", "end", "a", "e", "i", "o", "u"),
	LISTAIR("listair", "end", "a"),
	LKNER("lkner", "end", "a", "e", "i", "o", "u"),
	LLABY("llaby", "end", "a"),
	LLACE("llace", "end", "a", "e", "i", "o", "u"),
	LLAGHER("llagher", "end", "a"),
	LLAHAN("llahan", "end", "a"),
	LLBLADDER("llbladder", "end", "a"),
	LLDONE("lldone", "end", "e"),
	LLIAM("lliam", "end", "a", "e", "i", "o", "u"),
	LLIGAN("lligan", "end", "u"),
	LLIGATOR("lligator", "end", "a"),
	LLINAIRE("llinaire", "end", "a", "e", "i", "o", "u"),
	LLSPICE("llspice", "end", "a"),
	LLYGAG("llygag", "end", "o"),
	LLYWAG("llywag", "end", "a"),
	LOBALO("lobalo", "end", "a"),
	LOCH("loch", "end", "o"),
	LOIS("lois", "end", "a", "e", "i", "o", "u"),
	LOMON("lomon", "end", "o"),
	LONPAN("lonpan", "end", "e"),
	LONY("lony", "end", "e"),
	LOYSIA("loysia", "end", "a"),
	LOZAGA("lozaga", "end", "o"),
	LPACA("lpaca", "end", "a"),
	LPHOS("lphos", "end", "a", "e", "i", "o", "u"),
	LSAVERDE("lsaverde", "end", "a"),
	lSTOY("lstoy", "end", "o"),
	LRUS("lrus", "end", "a"),
	LYDE("lyde", "end", "a", "e", "i", "o", "u"),
	LYMPIA("lympia", "end", "o"),
	LZAC("lzac", "end", "a"),

	MADA("mafa", "end", "a"),
	MADEUS("madeus", "end", "a"),
	MAGUCHI("maguchi", "end", "a"),
	MANDCHEESE("mandcheese", "end", "a"),
	MAMOTO("mamoto", "end", "a"),
	MANUEL("manuel", "end", "e"),
	MARAJA("maraja", "end", "a"),
	MARIND("marind", "end", "a"),
	MATILLO("matillo", "end", "o"),
	MATO("mato", "end", "o"),
	MBINO("mbino", "end", "a", "e", "i", "o", "u"),
	MBLY("mbly", "end", "a", "e", "i", "o", "u"),
	MBOO("mboo", "end", "a"),
	MBOOZLE("mboozle", "end", "a"),
	MBOREE("mboree", "end", "a", "o", "u"),
	MBURGER("mburger", "end", "a", "e", "i", "o", "u"),
	MBUTAN("mbutan", "end", "a"),
	MBROISA("mbrosia", "end", "a"),
	MEGRANITE("megranite", "end", "o"),
	MELIA("melia", "end", "a"),
	MEMBERT("membert", "end", "a"),
	MENTINE("mentine", "end", "e"),
	MERIGO("merigo", "end", "a"),
	METHYST("methyst", "end", "a"),
	MHERST("mherst", "end", "a", "e", "i", "o", "u"),
	MHOCK("mhock", "end", "a"),
	MICHANGA("michanga", "end", "i"),
	MICHURRI("michurri", "end", "i"),
	MILAH("milah", "end", "a"),
	MILIUS("milius", "end", "a", "e", "i", "o", "u"),
	MILTON("milton", "end", "a"),
	MILY("mily", "end", "e"),
	MINGO("mingo", "end", "a", "e", "i", "o", "u"),
	MINGWAY("mingway", "end", "a", "e", "i", "o", "u"),
	MITRI("mitri", "end", "i"),
	MOGEN("mogen", "end", "i"),
	MONADE("monade", "end", "e"),
	MONGRASS("mongrass", "end", "e"),
	MPANADA("mpanada", "end", "e"),
	MPANELLE("mpanelle", "end", "a"),
	MPANZEE("mpanzee", "end", "i"),
	MPBELL("mpbell", "end", "a"),
	MPERNICKEL("mpernickel", "end", "a", "e", "i", "o", "u"),
	MPKIN("mpkin", "end", "u"),
	MPH("mph", "end", "a", "e", "i", "o", "u"),
	MPHODORUS("mphodorus", "end", "a", "e", "i", "o", "u"),
	MPUS("mpus", "end", "a", "e", "i", "o", "u"),
	MONTE("monte", "end", "a", "e", "i", "o", "u"),
	MQUAT("mquat", "end", "u"),
	MWELT("mwelt", "end", "u"),

	NALD("nald", "end", "a", "e", "i", "o", "u"),
	NALDO("naldo", "end", "a", "e", "i", "o", "u"),
	NANA("nana", "end", "a"),
	NANIGAN("nanigan", "end", "e"),
	NATHAN("nathan", "end", "a", "e", "i", "o", "u"),
	NAULT("nault", "end", "a", "e", "i", "o", "u"),
	NCEL("ncel", "end", "a", "e", "i", "o", "u"),
	NCER("ncer", "end", "a", "e", "i", "o", "u"),
	NCHILADA("nchilada", "end", "e"),
	NCHOVY("nchovy", "end", "a"),
	NCISCO("ncisco", "end", "a", "e", "i", "o", "u"),
	NCUBUS("ncubus", "end", "a", "e", "i", "o", "u"),
	NDARIN("ndarin", "end", "a"),
	NDEEP("ndeep", "end", "a"),
	NDELION("ndelion", "end", "a"),
	NDERBENDER("nderbender", "end", "e", "i"),
	NDERSON("nderson", "end", "a", "e", "i", "o", "u"),
	NDIANA("ndiana", "end", "a", "e", "i", "o", "u"),
	NDICE("ndice", "end", "a"),
	NDICOOT("ndicoot", "end", "a"),
	NDLER("ndler", "end", "a", "e", "i", "o", "u"),
	NDRAGORAS("ndragoras", "end", "a"),
	NDRASEKAR("ndrasekar", "end", "a"),
	NDREA("ndrea", "end", "a", "e", "i", "o", "u"),
	NDROMEDA("ndromeda", "end", "a"),
	NDWICH("ndwich", "end", "a", "e", "i", "o", "u"),
	NDYCANE("ndycane", "end", "a"),
	NEAPPLE("neapple", "end", "i"),
	NESHRAM("neshram", "end", "a", "e", "i"),
	NEFLU("neflu", "end", "i"),
	NEK("nek", "end", "a", "e", "i", "o", "u"),
	NELLA("nella", "end", "a", "e", "i", "o", "u"),
	NESTRONE("nestrone", "end", "i"),
	NEVIEVE("nevieve", "end", "e", "i"),
	NEYDEW("neydew", "end", "a", "e", "i", "o", "u"),
	NEYMAKER("neymaker", "end", "a", "e", "i", "o", "u"),
	NFLUENZA("nfluenza", "end", "i"),
	NGFISHER("ngfisher", "end", "a", "e", "i", "o", "u"),
	NGFELLOW("ngfellow", "end", "a", "e", "i", "o", "u"),
	NGA("nga", "end", "a", "e", "i", "o", "u"),
	NGELO("ngelo", "end", "a"),
	NGERBREAD("ngerbread", "end", "i"),
	NGERINE("ngerine", "end", "a"),
	NGO("ngo", "end", "a", "e", "i", "o", "u"),
	NGOSTEEN("ngosteen", "end", "a"),
	NGRID("ngrid", "end", "i"),
	NGUINI("nguini", "end", "i"),
	NGUIN("nguin", "end", "e"),
	NGUS("ngus", "end", "u"),
	NHILDE("nhilde", "end", "a", "e", "i", "o", "u"),
	NIAMANTAL("niamantal", "end", "o"),
	NICOTTI("nicotti", "end", "a"),
	NILLA("nilla", "end", "a"),
	NIPER("niper", "end", "u"),
	NITZEL("nitzel", "end", "a", "e", "i", "o", "u"),
	NIQUA("niqua", "end", "a", "e", "i", "o", "u"),
	NJAMIN("njamin", "end", "a", "e", "i", "o", "u"),
	NJO("njo", "end", "a", "o", "u"),
	NKLIN("nklin", "end", "a", "e", "i", "o", "u"),
	NKMEYER("nkmeyer", "end", "i"),
	NKO("nko", "end", "a", "e", "i", "o", "u"),
	NKY("nky", "end", "a", "e", "i", "o", "u"),
	NKYDORY("nkydory", "end", "a", "e", "i", "o", "u"),
	NKYKONG("nkykong", "end", "a", "o", "u"),
	NKYPANKY("nkypanky", "end", "a"),
	NNOCENCE("nnocence", "end", "i"),
	NNAMON("nnamon", "end", "i"),
	NNIBAL("nnibal", "end", "a"),
	NNIFER("nnifer", "end", "a", "e", "i", "o", "u"),
	NNIGAN("nnigan", "end", "a", "e", "i", "o", "u"),
	NNIVA("nniva", "end", "a", "e", "i", "o", "u"),
	NOCEROS("noceros", "end", "i"),
	NOCH("noch", "end", "e"),
	NONDORF("nondorf", "end", "a"),
	NPANYA("npanya", "end", "a"),
	NRIQUE("nrique", "end", "e", "i"),
	NSTON("nston", "end", "a", "e", "i", "o", "u"),
	NTACLAUS("ntaclaus", "end", "a"),
	NTALOPE("ntalope", "end", "a"),
	NTASIA("ntasia", "end", "a"),
	NTONETTE("ntonette", "end", "a", "e", "i", "o", "u"),
	NTRABASS("ntrabass", "end", "o"),
	NUMATI("numati", "end", "a"),
	NYA("nya", "end", "a", "e", "i", "o", "u"),
	NZALES("nzales", "end", "a", "e", "i", "o", "u"),
	NZO("nzo", "end", "a", "e", "i", "o", "u"),

	ODCHUCK("odchuck", "end", "o"),
	ODPECKER("odpecker", "end", "o"),
	OFUS("ofus", "end", "o"),
	OGALOO("oogaloo", "end", "o"),
	OK("ok", "end", "o"),
	OKABURRA("okaburra", "end", "o"),
	OLE("ole", "end", "o"),
	OLEY("oley", "end", "o"),
	ONEY("oney", "end", "o"),
	OSBALL("osball", "end", "o"),
	OTHBRUSH("othbrush", "end", "o"),
	OTHPASTE("othpaste", "end", "o"),
	OWULF("owulf", "end", "e"),

	PAYA("paya", "end", "a"),
	PHELIA("phelia", "end", "a", "e", "i", "o", "u"),
	PHETH("pheth", "end", "a"),
	PHIA("phia", "end", "o"),
	PHISTOPHOLES("phistopholes", "end", "a"),
	PHOCLES("phocles", "end", "o"),
	PHOMET("phomet", "end", "a"),
	PODILLA("podilla", "end", "a"),
	PORON("poron", "end", "o"),
	PPER("pper", "end", "a", "e", "i", "o", "u"),
	PPERGRINDER("ppergrinder", "end", "e"),
	PPERJACK("pperjack", "end", "e"),
	PPERMINT("ppermint", "end", "e"),
	PPERONCINO("pperoncino", "end", "e"),
	PPET("ppet", "end", "a", "e", "i", "o", "u"),
	PPINGS("ppings", "end", "a", "e", "i", "o", "u"),
	PPLEJACK("pplejack", "end", "a"),
	PPOSSUM("ppossum", "end", "o"),
	PPYCOCK("ppycock", "end", "o"),
	PRICORN("pricorn", "end", "a"),
	PRICOT("pricot", "end", "a"),
	PRIKA("prika", "end", "a"),
	PTISTA("ptista", "end", "a"),
	PTON("pton", "end", "a", "e", "i", "o", "u"),
	PYBARA("pybara", "end", "a"),

	QUAT("quat", "end", "a", "e", "i", "o", "u"),
	QUEFORT("quefort", "end", "a", "o"),
	QUILA("quila", "end", "e"),
	QUITA("shire", "end", "a", "e", "i", "o"),

	RABBA("rabba", "end", "a"),
	RABEAU("rabeau", "end", "a", "e", "i", "o", "u"),
	RAINE("raine", "end", "a", "o"),
	RAMAZOV("ramazov", "end", "a"),
	RAMBOLA("rabola", "end", "a", "e", "i", "o", "u"),
	RAPPA("rappa", "end", "a"),
	RARDO("rardo", "end", "a", "e", "i", "o", "u"),
	RATIO("ratio", "end", "a", "e", "i", "o", "u"),
	RAVIND("ravind", "end", "a"),
	RAWAY("raway", "end", "a"),
	RAYAN("rayan", "end", "o"),
	RBACOA("rbacoa", "end", "a"),
	RBARA("rbara", "end", "a", "o", "u"),
	RBECUE("rbecue", "end", "a", "o", "u"),
	RCESTER("rcester", "end", "a", "e", "i", "o", "u"),
	RCHIMEDES("rchimedes", "end", "a"),
	RCONE("rcone", "end", "a", "e", "i", "o", "u"),
	RCUPINE("rcupine", "end", "o"),
	RDAMOM("rdamom", "end", "a"),
	RDECAI("rdecai", "end", "o"),
	RDEJO("rdejo", "end", "e", "i"),
	RDELIA("rdelia", "end", "a", "e", "i", "o", "u"),
	RDIGRADE("rdigrade", "end", "a"),
	RDITA("rdita", "end", "o"),
	RDOCK("rdock", "end", "a", "e", "i", "o", "u"),
	RDYGURDY("rdygurdy", "end", "u"),
	REAU("reau", "end", "a", "e", "i", "o", "u"),
	RESCHI("reschi", "end", "a"),
	RFALLE("rfalle", "end", "a"),
	REDITH("redith", "end", "a", "e", "i", "o", "u"),
	REGANO("regano", "end", "o"),
	RFIELD("rfield", "end", "a", "e", "i", "o", "u"),
	RGARET("rgaret", "end", "a", "e", "i", "o", "u"),
	RGARITA("rgarita", "end", "a", "e", "i", "o", "u"),
	RGARINE("rgarine", "end", "a"),
	RGEON("rgeon", "end", "a", "e", "i", "o", "u"),
	RGENOST("rgenost", "end", "e"),
	RGONZOLA("rgonzola", "end", "o"),
	RGUL("rgul", "end", "e", "u"),
	RGUNDY("rgundy", "end", "u"),
	RJORAM("rjoram", "end", "a"),
	RIJUANAS("rijuanas", "a"),
	RIMOYA("rimoya", "end", "e", "i"),
	RIWINKLE("riwinkle", "end", "e"),
	RIZARD("rizard", "end", "a"),
	RKCHOP("rkchop", "end", "o"),
	RLANDO("rlando", "end", "a", "e", "i", "o", "u"),
	RLEMAGNE("rlemagne", "end", "a"),
	RLOS("rlos", "end", "a", "e", "i", "o", "u"),
	RLOTTE("rlotte", "end", "a", "e", "i", "o", "u"),
	RLOV("rlov", "end", "a", "e", "i", "o", "u"),
	RLSBERG("rlsberg", "end", "a", "e", "i", "o", "u"),
	RMALADE("rmalade", "end", "a", "e", "i", "o", "u"),
	RMELO("rmelo", "end", "a", "e", "i", "o", "u"),
	RMESAN("rmesan", "end", "a"),
	RMICELLI("rmicelli", "end", "e"),
	RMINE("rmine", "end", "a", "e"),
	RMORANT("rmorant", "end", "o"),
	RMOT("rmot", "end", "a"),
	RMSTRONG("rmstrong", "end", "a"),
	RMUR("rmur", "end", "u"),
	RMUZ("rmuz", "end", "u"),
	RNABUS("rnabus", "end", "a", "e", "i", "o", "u"),
	RNAISE("rnaise", "end", "a", "e", "i", "o", "u"),
	RNANDO("rnando", "end", "a", "e", "i", "o", "u"),
	RNARD("rnard", "end", "a", "e", "i", "o", "u"),
	RNELIUS("rnelius", "end", "a", "e", "i", "o", "u"),
	RNFLAKES("rnflakes", "end", "o"),
	RNIP("rnip", "end", "u"),
	RNITAS("rnitas", "end", "a"),
	RNSWOGGLE("rnswoggle", "end", "o"),
	ROCHE("roche", "end", "a"),
	RODOTUS("rodotus", "end", "e"),
	RONA("rona", "end", "a", "e", "i", "o", "u"),
	RONICA("ronica", "end", "a", "e", "i", "o", "u"),
	RORO("roro", "end", "a", "e", "i", "o", "u"),
	RPACCIO("rpaccio", "end", "a"),
	RPO("rpo", "end", "a", "e", "i", "o", "u"),
	RRACUDA("rracuda", "end", "a"),
	RRASCO("rrasco", "end", "a", "o", "u"),
	RRAULT("rrault", "end", "a", "e", "i", "o", "u"),
	RREAU("rreau", "end", "a", "e", "i", "o", "u"),
	RRITO("rrito", "end", "u"),
	RROWAY("rroway", "end", "a", "e", "i", "o", "u"),
	RSCHT("rscht", "end", "o"),
	RSERADISH("rseradish", "end", "o"),
	RSHMALLOW("arshmallow", "end", "a"),
	RSIMMON("rsimmon", "end", "e"),
	RSLEY("rsley", "end", "a"),
	RSNIP("rsnip", "end", "a"),
	RSULA("rsula", "end", "a", "e", "i", "o", "u"),
	RTALOTTI("rtalotti", "end", "a"),
	RTAZAR("rtazar", "end", "o"),
	RTELLINI("rtellini", "end", "o"),
	RTINBRAS("rtinbras", "end", "o"),
	RTHIK("rthik", "end", "a"),
	RTHOLOMEW("rtholomew", "end", "a", "e", "i", "o", "u"),
	RTICHOKE("rtichoke", "end", "a"),
	RTICUS("rticus", "end", "a", "o", "u"),
	RTIER("rtier", "end", "a", "e", "i", "o", "u"),
	RTILLA("rtilla", "end", "o"),
	RTOISE("rtoise", "end", "o", "u"),
	RTOLI("rtoli", "end", "a", "e", "i", "o", "u"),
	RTOUR("rtour", "end", "a", "e", "i", "o", "u"),
	RTRAND("rtrand", "end", "a", "e", "i", "o", "u"),
	RTRIDGE("rtridge", "end", "a"),
	RTUNATA("rtunata", "end", "o"),
	RUCH("ruch", "end", "a"),
	RUGULA("rugula", "end", "a"),
	RVETTE("rvette", "end", "o"),
	RWHAL("rwhal", "end", "a"),
	RYL("ryl", "end", "a", "e", "i", "o", "u"),
	RZIPAN("rzipan", "end", "a"),
	RZVLAK("rzvlak", "end", "a", "e", "i", "o", "u"),

	SAAC("saac", "end", "i"),
	SABELLA("sabella", "end", "a", "e", "i", "o", "u"),
	SADORA("sadora", "end", "a", "e", "i", "o", "u"),
	SAGNA("sagna", "end", "a"),
	SARIO("sario", "end", "o"),
	SCARGOT("scargot", "end", "e"),
	SCOE("scoe", "end", "a", "e", "i", "o", "u"),
	SDEMEANOR("sdemeanor", "end", "i"),
	SEIDON("seidon", "end", "o"),
	SEMARY("semary", "end", "o"),
	SERUP("serup", "end", "o"),
	SEVELT("sevelt", "end", "a", "e", "i", "o", "u"),
	SHINI("shini", "end", "a", "e", "i", "o", "u"),
	SHMAEL("shmael", "end", "a", "e", "i", "o", "u"),
	SHMERE("shmere", "end", "a"),
	SHNU("shnu", "end", "i"),
	SHUA("shua", "end", "o"),
	SHWACKER("shwacker", "end", "u"),
	SIMIR("simir", "end", "a"),
	SMODEL("smodel", "end", "a"),
	SPANOVA("spanova", "end", "a", "e", "i", "o", "u"),
	SPAR("spar", "end", "a"),
	SPARAGUS("sparagus", "end", "a"),
	SPUS("spus", "end", "a", "e", "i", "o", "u"),
	SRAEL("srael", "end", "e", "i"),
	SSAFRAS("ssafras", "end", "a"),
	SSAIN("ssain", "end", "a", "u"),
	SSARIAN("ssarian", "end", "a", "o", "u"),
	SSAVA("ssava", "end", "a"),
	SSICA("ssica", "end", "e"),
	SSIE("ssie", "end", "a", "e", "i", "o", "u"),
	SSILI("ssili", "end", "u"),
	SSINI("ssini", "end", "a", "e", "i", "o", "u"),
	STACHIO("stachio", "end", "i"),
	STADA("stada", "end", "o"),
	STAFA("stafa", "end", "o","u"),
	STANTE("stante", "end", "a", "e", "i", "o", "u"),
	STARD("stard", "end", "a", "e", "i", "o", "u"),
	STAROSSA("starossa", "end", "e"),
	STAROTH("starossa", "end", "a"),
	STEBAN("steban", "end", "a", "e", "i", "o", "u"),
	STOPHER("stopher", "end", "a", "e", "i", "o", "u"),
	STULE("stule", "end", "a", "e", "i", "o", "u"),

	TABAGA("tabaga", "end", "u"),
	TALINI("talini", "end", "a", "e", "i", "o", "u"),
	TANGA("tanga", "end", "a", "e", "i", "o", "u"),
	TANNIA("tannia", "end", "a", "e", "i", "o", "u"),
	TATO("tato", "end", "a", "o"),
	TATRON("tatron", "end", "a", "e", "i", "o", "u"),
	TCHEROO("tcheroo", "end", "i"),
	TCHOCOLATE("tchocolate", "end", "o"),
	TCOIN("tcoin", "end", "i"),
	TERMELON("termelon", "end", "a"),
	THELLO("thello", "end", "o"),
	THERFORD("therford", "end", "a", "e", "i", "o", "u"),
	THIUM("thium", "end", "a", "e", "i", "o", "u"),
	THRO("thro", "end", "a", "e", "i", "o", "u"),
	THESHEBA("thesheba", "end", "a", "e", "i", "o", "u"),
	TICIA("ticia", "end", "a", "e", "i", "o", "u"),
	TMEG("tmeg", "end", "u"),
	TNEY("tney", "end", "i", "u"),
	TNIP("tnip", "end", "a"),
	TONIO("tonio", "end", "a", "e", "i", "o", "u"),
	TTA("tta", "end", "a", "e", "i", "o", "u"),
	TTERMELON("ttermelon", "end", "i"),
	TTICELLI("tticelli", "end", "a", "e", "i", "o", "u"),
	TTICINI("tticini", "end", "a", "e", "i", "o", "u"),
	TTICUS("tticus", "end", "a", "e", "i", "o", "u"),
	TTONCHOPS("ttonchops", "end", "o", "u"),
	TTERCUPS("ttercups", "end", "o", "u"),

	UBLEDAY("ubleday", "end", "o"),
	UCLID("uclid", "end", "e"),
	UDOLINO("udolino", "end", "a"),
	UGENE("ugene", "end", "e"),
	UHAHA("uhaha", "end", "o"),
	UILLABAISSE("uillabiasse", "end", "o"),
	UILLON("uillon", "end", "o"),
	ULASH("ulash", "end", "o"),
	ULIFLOWER("uliflower", "end", "a"),
	UMKUCHEN("umkuchen", "end", "a"),
	UPHONIUM("uphonium", "end", "e"),
	USAGE("usage", "end", "a"),
	URDOUGH("urdough", "end", "o"),
	UTILUS("utilus", "end", "a"),
	UTREMAUNT("utremaunt", "end", "a"),

	VABEAN("vabean", "end", "a"),
	VACADO("vacado", "end", "a"),
	VAGAI("vagai", "end", "a"),
	VANAUGH("vanaugh", "end", "a"),
	VARY("vary", "end", "o"),
	VENGELINE("vangeline", "end", "e"),
	VERETT("verett", "end", "a", "e", "i", "o", "u"),
	VARIUS("varius", "end", "a", "e", "i", "o", "u"),
	VATAPPI("vatappi", "end", "a"),
	VECRAFT("vecraft", "end", "o"),
	VELIER("velier", "end", "a", "e", "i", "o", "u"),
	VENDER("vender", "end", "a"),
	VERGREEN("vergreen", "end", "e"),
	VERLORD("verlord", "end", "o"),
	VIATHAN("viathan", "end", "e"),
	VIKA("vika", "end", "a", "e", "i", "o", "u"),
	VINGTON("vington", "end", "a", "e", "i", "o", "u"),
	VINSKY("vinsky", "end", "a", "e", "i", "o", "u"),
	VIOLI("violi", "end", "a"),
	VITICUS("viticus", "end", "e"),

	WDER("wder", "end", "o"),
	WENHOEK("wenhoek", "end", "a", "e", "i", "o", "u"),
	WIFRUIT("wifruit", "end", "i"),
	WLER("wler", "end", "o"),
	WTHORNE("wthorne", "end", "a"),
	WYER("wyer", "end", "a", "o"),
	
	XANNE("xanne", "end", "a", "e", "i", "o", "u"),
	XFORD("xford", "end", "a", "e", "i", "o", "u"),
	XIMUS("ximus", "end", "a", "e", "i", "o", "u"),
	XLEY("xley", "end", "o", "u"),
	XOPHONE("xophone", "end", "a"),
	XTER("xter", "end", "a", "e", "i", "o", "u"),

	YAMOTO("yamoto", "end", "i"),
	YARDEE("yardee", "end", "o"),
	YENNE("yenne", "end", "a"),
	YLEE("ylee", "end", "a"),
	YOKO("yoko", "end", "i"),
	YONNAISE("yonnaise", "end", "a"),
	YOTE("yote", "end", "a"),

	ZAEMON("zaemon", "end", "o"),
	ZARUS("zarus", "end", "a", "e", "i", "o", "u"),
	ZATHOTH("zathoth", "end", "a"),
	ZAZEL("zazel", "end", "a"),
	ZBOT("zbot", "end", "a"),
	ZEBEL("zebel", "end", "a", "e", "i", "o", "u"),
	ZEKIEL("zekiel", "end", "a", "e", "i", "o", "u"),
	ZERAC("zerac", "end", "a"),
	ZMA("zma", "end", "a", "e", "i", "o", "u"),
	ZMO("zmo", "end", "a", "e", "i", "o", "u"),
	ZPACHO("zpacho", "end", "a"),
	ZZAPIE("zzapie", "end", "i"),
	ZZERELLA("zzerella", "end", "o"),
	ZZIWIG("zziwig", "end", "e"),

	;
	
	//This name and the end of this name fragment
	private final String me, endTag;
	
	//how likely is this segment to follow any of its predecessors?
	private final int weight;
	
	//A list of name fragments that this name fragment can follow.
	private final String[] canFollow;
	
	NameGenerator(String me, String endTag, int weight, String... canFollow) {
		this.me = me;
		this.endTag = endTag;
		this.weight = weight;
		this.canFollow = canFollow;
	}
	
	NameGenerator(String me, String endTag, String... canFollow) {
		this(me, endTag, 1, canFollow);
	}
	
	/**
	 * @param alliteration: Should the 2 names have the same first letter?
	 * @return a generated first and last name.
	 */
	public static String generateFirstLast(boolean alliteration) {

		Array<NameGenerator> possibleNexts = new Array<>();

		for (NameGenerator gen : NameGenerator.values()) {
			for (int i = 0; i < gen.canFollow.length; i++) {
				if ("start".equals(gen.canFollow[i])) {
					for (int j = 0; j < gen.weight; j++) {
						possibleNexts.add(gen);
					}
				}
			}
		}
		
		int randomIndex = MathUtils.random(possibleNexts.size - 1);
		NameGenerator next1 = possibleNexts.get(randomIndex);
		NameGenerator next2 = alliteration ? possibleNexts.get(randomIndex) : possibleNexts.get(MathUtils.random(possibleNexts.size - 1));
		
		String firstName = next1.me + generateName(next1.endTag);
		String lastName = next2.me + generateName(next2.endTag);
		
		firstName = firstName.substring(0, 1).toUpperCase() + firstName.substring(1);
		lastName = lastName.substring(0, 1).toUpperCase() + lastName.substring(1);
		return firstName + " " + lastName;
	}
	
	/**
	 * Recursive case. Randomly select a valid next name fragment
	 * @param prev: The name so far
	 * @return The generated name fragment, plus the rest of the name
	 */
	public static String generateName(String prev) {

		Array<NameGenerator> possibleNexts = new Array<>();
		
		//identify all possible next name fragments
		for (NameGenerator gen : NameGenerator.values()) {
			for (int i = 0; i < gen.canFollow.length; i++) {
				if (gen.canFollow[i].equals(prev)) {
					for (int j = 0; j < gen.weight; j++) {
						possibleNexts.add(gen);
					}
				}
			}
		}
		
		if (possibleNexts.isEmpty()) { return ""; }
		
		//pick a random possibly next fragment and add it.
		int randomIndex = MathUtils.random(possibleNexts.size - 1);
		NameGenerator next = possibleNexts.get(randomIndex);
		
		if ("end".equals(next.endTag)) {
			return next.me;
		} else {
			return next.me + generateName(next.endTag);
		}
	}
}
